...
public class CtrlHeaderExp : MonoBehaviour {
	...
	public void Initialize(UISlider _slExp ,UILabel _lbLevel , UILabel _lbAto , UILabel _lbMade , PopupAnimation _popupAnimation){
		...
		DataManager.Instance.user_data.WriteInt (DefineOld.USER_LEVEL, m_iLevel);
		//m_iLevel = DataManager.Instance.user_data.ReadInt (DefineOld.USER_LEVEL);
		m_iTotalExpTarget = DataManager.Instance.user_data.ReadInt (DefineOld.USER_TOTAL_EXP);
	...
	void Update () {
		...
		case STEP.LEVEL_UP:
			if (bInit) {
				...
				SoundManager.Instance.PlaySE ("se_levelup" , "https://s3-ap-northeast-1.amazonaws.com/every-studio/app/pocket/zoo/ver02/AssetBundles/" + AssetBundles.Utility.GetPlatformName() + "/assets/assetbundles/se");
			}
